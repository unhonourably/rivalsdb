...
export default function DevDiariesPage() {
  ...
  const [selectedEntry, setSelectedEntry] = useState<DevDiaryEntry | null>(null)
  ...
  const fetchData = async (signal?: AbortSignal) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load dev diaries')
    } finally {
      if (!signal?.aborted) {
        setLoading(false)
      }
      ...
      <main className="flex-1 pt-32 pb-24">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex flex-wrap items-start justify-between gap-6 mb-12">
            <div>
              <h1 className="text-4xl md:text-5xl font-semibold mb-3 bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent" style={{ fontFamily: 'var(--font-fredoka)' }}>Dev Diaries</h1>
              <p className="text-gray-400 max-w-3xl">Read behind-the-scenes insights and future plans from the Marvel Rivals team.</p>
            </div>
            <button
              type="button"
          ...
          {loading && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 mt-4">Fetching dev diaries...</p>
            </div>
          )}
              ...
              {entries.map(entry => (
                <div key={entry.id} className="group border border-white/10 bg-white/[0.02] rounded-2xl p-6 flex flex-col gap-4 hover:border-white/20 transition-all">
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <h2 className="text-lg space-y-2 font-semibold text-white mt-2 mb-2 leading-snug">{entry.title}</h2>
